...
import CustomAlert from '../common/CustomAlert';
  ...
  const OnBoarding = () => {
    ...
    const [inputValueCurrency, setInputValueCurrency] = useState<string>( i18n.language == 'en' ? currencyOptions[0] : currencyOptions[1]);
    ...
    const renderInformation = (
      <table className="border-collapse border border-gray-300 shadow-md rounded-lg w-full my-8">
        <tbody>
          {[
            { title: t('youWant'), value: selectedValue == KEY_CHOOSE_SOMETHING.SELL_SOMETHING ?  t('sellSomething') :  t('buySomething') },
    ...
    return (
      <div className='flex flex-col'>
        <div className='flex-grow p-6'>
          ...
          <CustomModalSuccess isOpen={isModalSuccess} titleSuccess={t('alertCheckMail')} textButtonConfirm={t('confirm')} onCloseModalSuccess={handleCloseModalSuccess}/>
          <CustomModal open={isOpenModal} textButtonEdit={t('edit')} textButtonOK={t('submit')} onClose={handleCloseModal} onOK={handleOKModal} title={t('hereInformation')} children={renderInformation}/>
        </div>
      </div>
    );
  };
